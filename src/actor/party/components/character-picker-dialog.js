...
export class CharacterPickerDialog extends Application {
	...
	static async show(title, characters, onSelect = noop, onCancel = noop) {
		const actors = characters.map(c => c instanceof Actor ? c : game.actors.get(c));
	...
	static get defaultOptions() {
		return mergeObject(super.defaultOptions, {
			...
			template: "systems/forbidden-lands/templates/actor/party/components/character-picker-dialog.hbs",
			width: 400,
			height: "auto",
			resizable: false,
		});
